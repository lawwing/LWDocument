...
public class PhotoView extends ImageView
{
    ...
    private PhotoInfo mFromPhotoInfo;
    ...
    private void onUp()
    {
        ...
        mTmpMatrix.postTranslate(cx - mHalfBaseRectWidth,
                cy - mHalfBaseRectHeight);
    ...
    private void doTranslateReset(RectF imgRect)
    {
        ...
        if (imgRect.width() <= mWidgetRect.width())
        {
            ...
            {
                tx = -(int) ((mWidgetRect.width() - imgRect.width()) / 2
                        - imgRect.left);
            }
        }
        else
        ...
        if (imgRect.height() <= mWidgetRect.height())
        {
            ...
            {
                ty = -(int) ((mWidgetRect.height() - imgRect.height()) / 2
                        - imgRect.top);
            }
        }
        else
    ...
    private ScaleGestureDetector.OnScaleGestureListener mScaleListener = new ScaleGestureDetector.OnScaleGestureListener()
    {
        ...
        public boolean onScale(ScaleGestureDetector detector)
        {
            ...
            mAnimaMatrix.postScale(scaleFactor,
                    scaleFactor,
                    detector.getFocusX(),
                    detector.getFocusY());
            executeTranslate();
                ...
                float distanceY)
        {
            ...
            else if (imgLargeWidth || hasMultiTouch || hasOverTranslate)
            {
                ...
                if (!hasMultiTouch)
                {
                    ...
                    {
                        distanceX = resistanceScrollByX(
                                mImgRect.left - mCommonRect.left,
                                distanceX);
                    }
                    if (distanceX > 0
                    ...
                    {
                        distanceX = resistanceScrollByX(
                                mImgRect.right - mCommonRect.right,
                                distanceX);
                    }
                }
            ...
            else if (imgLargeHeight || hasOverTranslate || hasMultiTouch)
            {
                ...
                if (!hasMultiTouch)
                {
                    ...
                    {
                        distanceY = resistanceScrollByY(
                                mImgRect.top - mCommonRect.top,
                                distanceY);
                    }
                    if (distanceY > 0
                    ...
                    {
                        distanceY = resistanceScrollByY(
                                mImgRect.bottom - mCommonRect.bottom,
                                distanceY);
                    }
                }
    ...
    public boolean canScrollVerticallySelf(float direction)
    {
        ...
        if (direction > 0 && Math.round(mImgRect.bottom)
    ...
    private class Transform implements Runnable
    {
        ...
        {
            mScaleScroller.startScroll((int) (form * 10000),
                    0,
                    (int) ((to - form) * 10000),
                    0,
                    mAnimaDuring);
        ...
        {
            mRotateScroller.startScroll(fromDegrees,
                    0,
        ...
        void withFling(float velocityX, float velocityY)
        {
            ...
            mFlingScroller.fling(mLastFlingX,
                    mLastFlingY,
                    (int) velocityX,
                    (int) velocityY,
                    minX,
        ...
        public void run()
        {
            ...
            else
            {
                ...
                if (imgLargeWidth)
                {
                    ...
                    {
                        mTranslateX -= (int) (mWidgetRect.width()
                                - mImgRect.right);
                    }
                    needFix = true;
                }
                ...
                if (imgLargeHeight)
                {
                    ...
                    {
                        mTranslateY -= (int) (mWidgetRect.height()
                                - mImgRect.bottom);
                    }
                    needFix = true;
                }
        ...
        private void applyAnima()
        {
            ...
            mAnimaMatrix
                    .postScale(mScale, mScale, mScaleCenter.x, mScaleCenter.y);
    ...
    public PhotoInfo getInfo()
    {
        ...
        int[] p = new int[2];
        getLocation(this, p);
        rect.set(p[0] + mImgRect.left,
                p[1] + mImgRect.top,
                p[0] + mImgRect.right,
                p[1] + mImgRect.bottom);
        return new PhotoInfo(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter,
    ...
    public static PhotoInfo getImageViewInfo(ImageView imgView)
    {
        ...
        RectF rect = new RectF(p[0] + imgRect.left, p[1] + imgRect.top,
                p[0] + imgRect.right, p[1] + imgRect.bottom);
        ...
        return new PhotoInfo(rect, imgRect, widgetRect, baseRect, screenCenter, 1, 0,
    ...
    public void animaFrom(PhotoInfo photoInfo)
    {
        if (isInit)
        {
            ...
            float scaleX = photoInfo.mImgRect.width() / mine.mImgRect.width();
            float scaleY = photoInfo.mImgRect.height() / mine.mImgRect.height();
            float scale = scaleX < scaleY ? scaleX : scaleY;
            
            float ocx = photoInfo.mRect.left + photoInfo.mRect.width() / 2;
            float ocy = photoInfo.mRect.top + photoInfo.mRect.height() / 2;
            ...
            mAnimaMatrix.postRotate(photoInfo.mDegrees, ocx, ocy);
            executeTranslate();
            
            mScaleCenter.set(ocx, ocy);
            mRotateCenter.set(ocx, ocy);
            
            mTranslate.withTranslate(0,
                    0,
                    (int) -(ocx - mcx),
                    (int) -(ocy - mcy));
            mTranslate.withScale(scale, 1);
            mTranslate.withRotate((int) photoInfo.mDegrees, 0);
            
            if (photoInfo.mWidgetRect.width() < photoInfo.mImgRect.width()
                    || photoInfo.mWidgetRect.height() < photoInfo.mImgRect.height())
            {
                float clipX = photoInfo.mWidgetRect.width() / photoInfo.mImgRect.width();
                float clipY = photoInfo.mWidgetRect.height()
                        / photoInfo.mImgRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;
                
                ClipCalculate c = photoInfo.mScaleType == ScaleType.FIT_START
                        ? new START()
                        : photoInfo.mScaleType == ScaleType.FIT_END ? new END()
                                : new OTHER();
                
                mTranslate.withClip(clipX,
                        clipY,
                        1 - clipX,
                        1 - clipY,
                        mAnimaDuring / 3,
        ...
        {
            mFromPhotoInfo = photoInfo;
            mInfoTime = System.currentTimeMillis();
        }
    }
    
    public void animaTo(PhotoInfo photoInfo, Runnable completeCallBack)
    {
        if (isInit)
        {
            mTranslate.stop();
            ...
            float tcx = photoInfo.mRect.left + photoInfo.mRect.width() / 2;
            float tcy = photoInfo.mRect.top + photoInfo.mRect.height() / 2;
            ...
            // 缩放
            float scaleX = photoInfo.mImgRect.width() / mBaseRect.width();
            float scaleY = photoInfo.mImgRect.height() / mBaseRect.height();
            float scale = scaleX > scaleY ? scaleX : scaleY;
            ...
            mTranslate.withTranslate(0,
                    0,
                    (int) (tcx - mScaleCenter.x),
                    (int) (tcy - mScaleCenter.y));
            mTranslate.withScale(mScale, scale);
            mTranslate.withRotate((int) mDegrees,
                    (int) photoInfo.mDegrees,
                    mAnimaDuring * 2 / 3);
            
            if (photoInfo.mWidgetRect.width() < photoInfo.mRect.width()
                    || photoInfo.mWidgetRect.height() < photoInfo.mRect.height())
            {
                float clipX = photoInfo.mWidgetRect.width() / photoInfo.mRect.width();
                float clipY = photoInfo.mWidgetRect.height() / photoInfo.mRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;
                
                final float cx = clipX;
                final float cy = clipY;
                final ClipCalculate c = photoInfo.mScaleType == ScaleType.FIT_START
                        ? new START()
                        : photoInfo.mScaleType == ScaleType.FIT_END ? new END()
                                : new OTHER();
