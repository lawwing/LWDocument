...
public class CalendarUtils
{
    ...
    private void initAllHolidays(Context context)
    {
        try
        {
            ...
            sAllHolidays = new Gson().fromJson(baos.toString(),
                    new TypeToken<Map<String, int[]>>()
                    {
                    }.getType());
        }
        catch (IOException e)
    ...
    public List<Integer> removeTaskHints(int year, int month,
            List<Integer> days)
    {
        String key = hashKey(year, month);
        List<Integer> hints = sUtils.sMonthTaskHint.get(key);
     ...
     * @return
     */
    public static int getWeeksAgo(int lastYear, int lastMonth, int lastDay,
            int year, int month, int day)
    {
        ...
        float v = (end.getTimeInMillis() - start.getTimeInMillis())
                / (3600 * 1000 * 24 * 7 * 1.0f);
        return (int) (v - 1);
    }
     ...
     * @return
     */
    public static int getMonthsAgo(int lastYear, int lastMonth, int year,
            int month)
    {
    ...
    public static String getHolidayFromSolar(int year, int month, int day)
    {
        ...
        else if (month == 3)
        {
            ...
            else if (day >= 4 && day <= 6)
            {
                if (year <= 1999)
                {
                    int compare = (int) (((year - 1900) * 0.2422 + 5.59)
                            - ((year - 1900) / 4));
                    if (compare == day)
                    {
                        message = "清明节";
                ...
                {
                    int compare = (int) (((year - 2000) * 0.2422 + 4.81)
                            - ((year - 2000) / 4));
                    if (compare == day)
                    {
                        message = "清明节";
